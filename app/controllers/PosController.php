...
class PosController
{
    ...
    public function search(): void
    {
        ...
        $unit = Database::fetch(
            "SELECT u.id AS unit_id, u.serial_number,
                    p.id, p.name, p.sku, p.sell_price, p.is_serialized
        ...
        if ($unit) {
            json_response([[
                ...
                'serialized'  => true,
                'stock'       => 1,
                'units'       => [['id' => (int) $unit['unit_id'], 'serial' => $unit['serial_number']]],
            ]]);
        }
        ...
        $rows = Database::fetchAll(
            "SELECT p.id, p.name, p.sku, p.sell_price, p.is_serialized,
        ...
        foreach ($rows as $r) {
            ...
            if ($serialized) {
                $units = Database::fetchAll(
                    "SELECT id, serial_number FROM inventory_units
                      WHERE product_id = ? AND status = 'in_stock' ORDER BY id LIMIT 60",
                    [$r['id']]
                );
                $units = array_map(fn ($u) => ['id' => (int) $u['id'], 'serial' => $u['serial_number']], $units);
            }
            $out[] = [
                'id'          => (int) $r['id'],
                'name'        => $r['name'],
